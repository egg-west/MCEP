...
from MCEP.agents import TD3BCEPLearner
from MCEP.data import D4RLDataset
from MCEP.evaluation import evaluate, evaluate_normalized_state, evaluate_surrogate_normalized_state
...
config_flags.DEFINE_config_file(
    "config",
    "configs/offline_config.py:td3bcep_adroit",
...
def main(_):
    wandb.init(project=f"adroit_offline_td3bcep_bcNorm{FLAGS.config.model_config.use_bcNorm}-{FLAGS.config.model_config.bp_alpha}-{FLAGS.config.model_config.tp_alpha}",
               group="td3bcep-"+str(FLAGS.config.model_config.bp_bc_loss_weight)+"-"+\
                     str(FLAGS.config.model_config.tp_bc_loss_weight)+"-"+FLAGS.env_name,
               name = str(FLAGS.seed))

...
from MCEP.agents import AWACEPLearner
from MCEP.data import D4RLDataset
from MCEP.evaluation import evaluate, evaluate_normalized_state, evaluate_surrogate_normalized_state, evaluate_surrogate
...
flags.DEFINE_boolean("bc_norm", None, "Use bc_norm for evaluation policy.")
...
config_flags.DEFINE_config_file(
    "config",
    "configs/offline_adroit_config.py:awacep_adroit",
    "File path to the training hyperparameter configuration.",
    lock_config=False,
)

def main(_):
    wandb.init(project=f"adroit_offline_AWACEP-ORIGINSTATE-{FLAGS.config.model_config.tilde_lambda}-{FLAGS.config.model_config.evaluation_lambda}",
               group=f"awacep-{FLAGS.config.model_config.tilde_lambda}-{FLAGS.config.model_config.evaluation_lambda}-{FLAGS.env_name}",
               name=str(FLAGS.seed))
    wandb.config.update(FLAGS)

    if FLAGS.bc_norm != None:
        FLAGS.config.model_config.bc_norm = FLAGS.bc_norm
    ...
    for i in tqdm.tqdm(
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            surrogate_info = evaluate_surrogate(agent, env, FLAGS.eval_episodes)
            surrogate_info["return"] = env.get_normalized_score(surrogate_info["return"]) * 100.0
            for k, v in surrogate_info.items():
